        ...
        return new Promise((resolve, reject) => {
            ...
            this.peer.on('error', (err) => {
                ...
                if (err.type === 'unavailable-id') {
                    ...
                    // If we're offline, we might still want the app to "start"
                    // We can resolve with a special status or just log it
                    console.warn("Peer server unreachable. P2P features will be disabled.");
                    resolve(userId); // Resolve anyway so onboarding completes
                } else {
                    reject(err);
                }
            });
